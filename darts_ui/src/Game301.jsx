...
function Game301(props) {
  ...
  const getDartZoneId = (dart) => {
    if (dart.double) {
      return `double-${dart.section}`;
    } else if (dart.triple) {
      return `triple-${dart.section}`;
    } else {
      return `simple-${dart.section}`;
    }
  };

  const getDartHumanReadable = (dart) => {
    if (dart.double) {
      return `double ${dart.section}`;
    } else if (dart.triple) {
      return `triple ${dart.section}`;
  ...
  const onDart = (value) => {
    ...
    setHighlightZone(getDartZoneId(value));
    ...
    <>
      <div className="two-columns">
                ...
                <div className="dartsDetails">
                  {darts.map((dart, index) => {
                    if (index > darts.length - 10) {
                      return (
                        <div key={index}>
                          {dart.player} : {dart.value.double ? "double" : ""}
                          {dart.value.triple ? "triple" : ""}
                          {dart.value.section}
                        </div>
                      );
                    }
        ...
        <div>
          ...
          <h3>
            Reste {remianingDarts}
            {remianingDarts > 1 ? " fléchettes" : " fléchette"}
          </h3>
          <h4>
            Dernier lancer :{" "}
            {darts.length > 0
              ? `${getDartHumanReadable(darts[darts.length - 1].value)} par ${
                  darts[darts.length - 1].player
                }`
              : "Aucune"}
          </h4>
          <Dartboard size="300px" highlightZone={highlightZone} />
          ...
          <button style={{ marginLeft: "0.5rem" }} onClick={() => nextPlayer()}>
            Passer
          </button>
          <button
            style={{ marginLeft: "0.5rem" }}
            onClick={() => setOpenCorrectScore(true)}
          >
            Corriger
          </button>
        </div>
        ...
        {openCorrectScore && (
          <div className="popup--background">
            <div className="popup--content">
              ...
              <h4>
                Flèche détectée :{" "}
                {darts[darts.length - 1].value.double ? "double" : ""}{" "}
                {darts[darts.length - 1].value.triple ? "triple" : ""}{" "}
                {darts[darts.length - 1].value.section}
              </h4>
              ...
              <button
                className="button--sm"
                onClick={() => setOpenCorrectScore(false)}
              >
                Fermer
              </button>
            </div>
